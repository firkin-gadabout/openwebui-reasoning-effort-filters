"""
title: Reasoning Effort (Astra)
id: reasoning_effort_astra
description: Per-chat reasoning effort for GPT-6 Astra. Supports low/medium/high/xhigh/max; 'none' is not offered because Astra rejects it (HTTP 400).
author: branden
version: 1.3.0
...
class Filter:
    ...
    class UserValves(BaseModel):
        effort: Literal["default", "low", "medium", "high", "xhigh", "max"] = Field(
            default="default",
            description=(
                "How much to think. 'default' = send nothing (model default). "
    ...
    async def request(
        ...
        if ASTRA_MODEL not in model_id:
            log.info(
                "Reasoning: attached to Astra but request targets '%s'; not injecting",
                model_id,
            )
            return body
        ...
        if effort == "none":
            # Unreachable via UI (not in the Literal); guard for stale saved user valves.
        ...
        # Idempotent, so safe to run on every tool-loop hop.
